# 个人作品集站（Next.js 15 + Prisma + Tailwind）

面向内容/新媒体/增长方向的全栈个人站点：前台（首页/关于/经历/作品集/联系）+ 后台可维护内容 + Postgres 数据存储，可一键部署到 Vercel。
...
<!-- 原静态预览（static-preview/index.html）已移除，保持项目更轻量。 -->
...
媒体文件放 `public/media/` 后，在后台填写相对路径即可（如 `/media/showreel.mp4`）。不想进后台也可直接替换同名文件。
...
- **关于/经历/作品/联系**：对应 `app/(site)/*/page.tsx`。
